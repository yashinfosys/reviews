...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (isDemoMode()) return NextResponse.json({ ok: true, id, developerBranding, demoMode: true });

  const business = await prisma.business.update({ where: { id }, data: { developerBranding } });
