...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  ...
  const admin = await prisma.user.findFirst({ where: { businessId: params.id, role: Role.BUSINESS_ADMIN } });
  ...
  await writeAuditLog({ businessId: params.id, userId: user.id, action: "BUSINESS_ADMIN_PASSWORD_RESET", entity: "User", entityId: admin.id });
